...
`person` и `date` здесь — не выдумка: это идентификаторы, которые вы задавали
каждому полю. Их видно в списке полей, в строке «Идентификатор». Пишутся они
латиницей и начинаются с буквы, дальше можно цифры и подчёркивание:
`date_created` и `person2` годятся, `2date` и «дата» — нет.
...
**Дата прямо в имени.** В имя и папку подставляются не только ответы, но и
текущее время — `{{date:ФОРМАТ}}`.

    Имя заметки:  {{date:YYYYMMDDHHmmss}}

даст `20260328232135`: год, месяц, день, часы, минуты, секунды — четырнадцать
цифр подряд. Формат складывается из букв:
...
| `MM` | месяц — 03 |
| `DD` | день — 28 |
| `HH` | часы, 24-часовые — 23 |
| `mm` | минуты — 21 |
| `ss` | секунды — 35 |

Всё, что не из этой таблицы, остаётся как написано: `{{date:YYYY-MM-DD}}`
даст `2026-03-28`. Папку так тоже удобно собирать —
`Дневник/{{date:YYYY}}/{{date:MM}}` разложит заметки по годам и месяцам без
единого поля в форме.
...
время, `{{datetime}}` — и то и другое. Понимает и `{{date:ФОРМАТ}}` — тот же
формат, что в имени заметки. Поле откроется уже заполненным, а поправите его
в тот редкий раз, когда встреча была вчера.
...
Значение спрашивается тем же, чем спрашивает само поле, на которое условие
смотрит: у списка — выбором из его вариантов, у даты и времени — календарём,
у заметки и папки — подсказкой из хранилища. Угадывать написание варианта или
формат даты не нужно.
...
Дата и время: `{{date:ФОРМАТ}}` — из букв `YYYY`, `YY`, `MM`, `DD`, `HH`,
`mm`, `ss`. Работает везде, где работают подстановки, включая значение по
умолчанию. Короткие записи `{{today}}`, `{{now}}` и `{{datetime}}` остались и
равны `{{date:YYYY-MM-DD}}`, `{{date:HH:mm}}` и `{{date:YYYY-MM-DDTHH:mm}}`.
