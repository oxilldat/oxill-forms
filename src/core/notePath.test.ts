...
test("имя собирается из полей", () => {
  assert.equal(
    renderNoteName("{{author}} — {{title}}", data, "Книга"),
...
test("незаполненное поле не оставляет висячих разделителей", () => {
  ...
  assert.equal(
    renderNoteName("{{empty}} — {{title}}", data, "Книга"),
    "Война и мир",
  );
  assert.equal(
    renderNoteName("{{title}} — {{empty}}", data, "Книга"),
    "Война и мир",
  );
});
...
test("косая черта в пути остаётся разделителем, прочий мусор режется", () => {
  ...
  assert.equal(
    renderNoteFolder("Книги/{{genre}}", risky),
    "Книги/Роман- часть 1",
  );
});
...
// Момент времени фиксируем: иначе тест зависел бы от часов машины.
const moment = new Date(2026, 2, 28, 23, 21, 35);

test("имя заметки может быть штампом времени", () => {
  assert.equal(
    renderNoteName("{{date:YYYYMMDDHHmmss}}", data, "Запасное", moment),
    "20260328232135",
  );
});

test("двоеточие из формата времени не уезжает в имя файла", () => {
  // В имени файла двоеточие запрещено, поэтому оно становится дефисом —
  // как и любой другой запрещённый символ.
  assert.equal(
    renderNoteName("{{date:HH:mm}}", data, "Запасное", moment),
    "23-21",
  );
});

test("папка собирается из даты без единого поля формы", () => {
  assert.equal(
    renderNoteFolder("Дневник/{{date:YYYY}}/{{date:MM}}", data, moment),
    "Дневник/2026/03",
  );
});

test("дата и поле уживаются в одном имени", () => {
  assert.equal(
    renderNoteName("{{date:YYYY-MM-DD}} {{author}}", data, "Запасное", moment),
    "2026-03-28 Лев Толстой",
  );
});
